      ...
      <div className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto bg-[#0d0520] border border-purple-800/30 rounded-[32px] shadow-2xl">
        <div className="sticky top-0 z-10 bg-[#0d0520] border-b border-purple-900/30 px-8 py-6 flex items-center justify-between rounded-t-[32px]">
          <div>
            <h2 className="text-2xl font-black text-white tracking-tight">
              New <span className="text-orange-500">Course</span>
            </h2>
            <p className="text-purple-500 text-xs font-semibold mt-0.5">
              Add a new class to the 2026 curriculum
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-8 flex flex-col gap-6">
          <Field label="Course Title" error={errors.title}>
            <input
              type="text"
              placeholder="e.g. AI Prompt Engineering for Kids"
              value={form.title}
              onChange={set("title")}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Field label="Category" error={errors.category}>
              <div ref={categoryRef} className="relative">
                ...
                >
                  <span className={form.category ? "text-white" : "text-purple-700"}>
                    {form.category || "Select a category"}
                  </span>
                  <ChevronDown
                    size={16}
                    className={`text-purple-500 transition-transform ${categoryOpen ? "rotate-180" : ""}`}
                  />
                </button>
                {categoryOpen && (
            ...
            <Field label="Price (Ksh)" error={errors.price}>
              <div className="relative">
                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-[10px] font-black text-orange-500">
                  KSH
                </span>
                <input
                  type="number"
            ...
            <Field label="Status">
              <div className="flex items-center gap-3 h-[46px] px-4 bg-purple-950/20 border border-purple-800/30 rounded-xl">
                <button
                  ...
                  className={`relative w-11 h-6 rounded-full transition-colors ${form.is_active ? "bg-orange-500" : "bg-purple-900"}`}
                >
                  <span
                ...
                <span className={`text-sm font-bold ${form.is_active ? "text-orange-400" : "text-purple-600"}`}>
          ...
          <div className="flex gap-3 pt-2">
            ...
            >
              {success ? (
                <><CheckCircle2 size={18} /><span>Created!</span></>
              ) : submitting ? (
                <><Loader2 size={18} className="animate-spin" /><span>Saving...</span></>
              ) : (
                <><Plus size={18} strokeWidth={3} /><span>Publish Course</span></>
              )}
            </button>
          </div>
        </form>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
        <div>
          <h1 className="text-3xl font-black text-white tracking-tight">
            Course <span className="text-orange-500">Inventory</span>
          </h1>
          <p className="text-purple-400/60 text-sm font-medium mt-1">
            Manage and audit the 2026 curriculum.
          </p>
        </div>
        <button
          onClick={() => setModalOpen(true)}
          className="bg-orange-500 hover:bg-orange-600 text-white font-black px-6 py-4 rounded-2xl flex items-center gap-2 transition-all shadow-lg active:scale-95"
        >
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 lg:pb-0 scrollbar-hide">
          {categories.map((cat) => (
            <button
              ...
              className={`px-6 py-4 rounded-2xl font-bold text-xs uppercase tracking-widest transition-all whitespace-nowrap border ${
                activeCategory === cat
                  ? "bg-purple-600 border-purple-500 text-white shadow-lg shadow-purple-500/20"
                  : "bg-purple-950/20 border-purple-800/30 text-purple-500 hover:border-purple-700"
              }`}
            >
              {cat}
            </button>
              ...
              {loading ? (
                ...
                filteredCourses.map((course: Course) => (
                  <tr key={course.id} className="hover:bg-purple-500/5 transition-colors group">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="max-w-[240px]">
                          <div className="text-white font-bold text-sm truncate group-hover:text-orange-400 transition-colors">
                            {course.title}
                          </div>
                          <div className="text-purple-500 text-[11px] truncate mt-0.5">
                            {course.description}
                          </div>
                        </div>
                      </div>
                    ...
                    <td className="px-6 py-4">
                      {/* ✅ Fixed: was text-black on dark bg */}
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        {/* ✅ Edit → navigates to course detail for editing */}
                        <button
                          onClick={() => navigate(`/admin/courses/${course.id}`)}
                          className="p-2 hover:bg-purple-800/30 text-purple-400 hover:text-white rounded-lg transition-all"
                          title="Edit curriculum"
                        >
                          <Edit3 size={16} />
                        </button>
                        {/* ✅ Eye → also navigates to course detail (same page) */}
                        <button
                          onClick={() => navigate(`/admin/courses/${course.id}`)}
                          className="p-2 hover:bg-purple-800/30 text-purple-400 hover:text-white rounded-lg transition-all"
                        ...
                        {/* Delete — to be wired once delete thunk is added */}
                        <button
                          className="p-2 hover:bg-rose-500/10 text-rose-500/60 hover:text-rose-400 rounded-lg transition-all"
                          title="Delete"
                        >
                          <Trash2 size={16} />
                        </button>
